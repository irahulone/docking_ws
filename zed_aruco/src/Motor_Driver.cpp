...
#define Y_OFFSET 60
#define ANGULAR_COMPENSATION_DIST 450
...
public:
  ...
  : Node("motor_driver")
  {
    ...
    subscription_ = this->create_subscription<zed_aruco::msg::ArucoPose>(SUBSCRIBER_TOPIC, QUEUE_LENGTH, std::bind(&Motor_Driver::read_aruco_position, this, std::placeholders::_1));
  }
...
private:
  ...
  rclcpp::Subscription<zed_aruco::msg::ArucoPose>::SharedPtr subscription_;

  void read_aruco_position(const zed_aruco::msg::ArucoPose::SharedPtr msg)
  {
    /*
    if (!msg->data) {
      pos_y = 0;
    ...
    //RCLCPP_INFO(this->get_logger(), "Aruco ID: %d", msg->id);
    //RCLCPP_INFO(this->get_logger(), "Aruco Position: x = %g", msg->position.x);
    //RCLCPP_INFO(this->get_logger(), "Aruco Position: y = %g", msg->position.y);
    ...
    //RCLCPP_INFO(this->get_logger(), "Buffer: %d", std::stoi(buffer));
    ...
    z_angular = msg->orientation.euler.z;
    if (std::abs(msg->position.y)<600 && msg->id == 0){
    ...
    pub = true;
    //RCLCPP_INFO(this->get_logger(), "Aruco Position: w = %g", msg->orientation.w);
    //RCLCPP_INFO(this->get_logger(), "Aruco Position: x = %g; y = %g; z = %g", msg->position.x, msg->position.y, msg->position.z);
    //RCLCPP_INFO(this->get_logger(), "Aruco Orientation: x = %g; y = %g; z = %g", msg->orientation.x, msg->orientation.y, msg->orientation.z);
  }
  void publish_motor_values(){
    ...
    std::cout<<"Y offset: "<<(Y_OFFSET + (((pos_x - 400) + 130) * tan(z_angular)))<<std::endl;
    // Enable angular compensation at set distance
    ey = (pos_x < ANGULAR_COMPENSATION_DIST && ANGULAR_COMP) ? (Y_OFFSET + (((pos_x - 400) + 130) * tan(z_angular)))-pos_y : Y_OFFSET - pos_y;
