...
// Maximum search radius in pixels for finding valid 3d point
...
// Function to detect ArUco markers, refine corners, and display the image
...
public:
  ...
  : Node("_publisher")
  {
		...
		init_params.depth_mode = DEPTH_MODE; // Use ULTRA depth mode
		...
		image_ocv = cv::Mat(image_zed.getHeight(), image_zed.getWidth(), CV_8UC4, image_zed.getPtr<sl::uchar1>(sl::MEM::CPU));	//assuming opencv uses cuda in the back
...
private:
  ...
  void publish_pose(){
		...
		printRotation(rotation_matrix);
		//printRotationAsEulerAngles(rotation_matrix);
		...
		if (ids.size() > 0)
		{
			...
			detectAndDisplay(image_ocv_rgb, corners, ids);
			for (int i = 0; i < ids_size; i++)
			{
				...
				*/
				...
				if (find_plane_status == sl::ERROR_CODE::SUCCESS && PLANE_ORIENTATION) {
					...
					quaternions[0] = orientation(3); // w component
					quaternions[1] = orientation(0); // x component
					quaternions[2] = orientation(1); // y component
					quaternions[3] = orientation(2); // z component
